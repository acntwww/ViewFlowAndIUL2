...
public class ViewFlow extends AdapterView<Adapter> {
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
            ...
            xMoved = xMoved && Math.abs(deltaX) > Math.abs(deltaY);
            yMoved = yMoved && Math.abs(deltaX) < Math.abs(deltaY);
            if (xMoved  && !yMoved) {
                requestParentListViewToNotInterceptTouchEvents(true);
    ...
    private void requestParentListViewToNotInterceptTouchEvents(Boolean disallowIntercept) {
        ...
        mShouldAuto = !disallowIntercept;
    }


    private boolean mShouldAuto = true;
    public boolean shouldAutoRound() {
        return mShouldAuto;
    }
}
